...
import { getOrCreateSession, SESSION_COOKIE } from '@/lib/services/session.service';

export async function POST(req: NextRequest) {
  const { qrToken } = await req.json();
  if (!qrToken) return NextResponse.json({ ok: false, error: 'missing_qr' }, { status: 400 });

  const result = await getOrCreateSession(qrToken);
  if (!result.ok) return NextResponse.json(result, { status: 400 });

  // Only write a new cookie when a new session was created.
  // Reused sessions already have a valid cookie — do not overwrite it.
  if (result.isNew) {
